...
从去年年底开始笔者决定深入 WebAssembly（接来下使用缩写 Wasm）这门技术，在读[《WebAssembly 原理与核心技术》](https://book.douban.com/subject/35233448/)这本书的过程中（这本书详细讲解了 Wasm 的解释器和虚拟机的工作原理以及实现思路），萌生了实现一个 Wasm 解释器的想法，于是就有了这个项目。接下来我们就直奔主题，看下到底如何实现一个 Wasm 解释器。

## Wasm 背景知识

在具体阐述解释器实现过程之前，首先介绍以下 Wasm 相关的背景知识。

### 什么是 Wasm

Wasm 是一种底层类汇编语言，能在 Web 平台上以趋近原生应用的速度运行。C/C++/Rust/Go 等语言将 Wasm 作为编译目标语言，可以将已有的代码移植到 Web 平台中运行，以提升代码复用度。

<img src="https://i.loli.net/2021/08/16/cVHFxMk5JAaL38S.png" width=400/>
...
<img src="https://i.loli.net/2021/08/16/E3296pwa8rxXDeQ.png" width=600/>

而内存格式和具体的 Wasm 解释器的实现有关，例如本项目的内存格式大致如下（在后面执行阶段部分会详细讲解）：

<img src="https://i.loli.net/2021/08/16/p59tyDCn2joF7uv.png" width=1000/>
...
<img src="https://i.loli.net/2021/08/16/31JpiBPgvzh7bXM.png" width=500/>

### Wasm 应用

Wasm 目前已经在浏览器端的音视频、IDE、设计工具、游戏等，以及非浏览器端的区块链、Serverless 等领域有一定的应用。如果想要了解更多有关 Wasm 应用的内容，可以关注笔者的另一个 GitHub 仓库：

[https://github.com/mcuking/Awsome-Wasm-Applications](https://github.com/mcuking/Awsome-Wasm-Applications)
...
2. **验证阶段**：对模块进行静态分析，确保模块的结构满足规范要求，且函数的字节码没有不良性为（例如调用不存在的函数）。
3. **执行阶段**：进一步分为**实例化**和**函数调用**两个阶段。

<img src="https://i.loli.net/2021/08/16/jfEZKG9QTIVAeLv.png" width=400/>
...
<img src="https://i.loli.net/2021/08/16/ihKrCBZkS2LfImt.png" width=600/>

也就是说 Wasm 二进制文件会将程序的信息按照不同类型分别存储在对应的段中：
...
> LEB128 有两个变体，分别用来编码无符号整数和有符号整数，更多细节可查阅项目中 `utils.c` 文件的 `read_LEB` 函数。

最后展示下解码阶段对应的部分实际代码截图如下：

<img src="https://i.loli.net/2021/08/20/s7uF3XMUEcGjLOY.png" width=600/>

更多细节建议查阅项目中 `module.c` 文件的 `load_module` 函数，其中有丰富的注释讲解。

## 执行阶段

经过了上面的解码阶段，我们可以从 Wasm 二进制文件中得到涵盖执行阶段所需要的全部信息的内存格式，接下来我们来一起探索如何基于上面的内存格式实现执行阶段。在正式开始之前，首先需要介绍下栈式虚拟机的相关知识作为铺垫。

官网对 Wasm 的定义--**Wasm 是基于栈式虚拟机的二进制指令格式**。也就是说 Wasm 不仅是一门编程语言，也是一套虚拟机体系结构规范。那么什么是虚拟机，什么又是栈式虚拟机呢？
...
执行指令流中的一条条指令，就是不断循环执行上面的三个步骤。循环执行的过程中需要有一个标志来记录当前已经执行到哪一条指令，也就是**程序计数器 PC (Program Count)**。

- **程序计数器**：用于记录下一条待执行指令的地址。
  由于虚拟机每次只执行一条指令，执行多个指令后，就相当于给程序中的指令统计数量了，因此被称为程序计数器。
  ARM 架构的 CPU 存储下一条待执行指令地址的寄存器叫 PC(Program Count)，X86 架构的 CPU 存储下一条待执行指令地址的寄存器称为 IP(Instruction Pointer)。
...
下图是 Wasm 部分指令的操作码助记符的枚举，完成版请查阅项目中 `opcode.h` 文件。

<img src="https://i.loli.net/2021/08/17/yIGTJfNtaV4FO1S.png" width=400/>
...
<img src="https://i.loli.net/2021/08/17/TicKxgytZl4VRmX.png" width=300/>
...
- `i32.const 3`：表示压入索引为 3 的局部变量到操作数栈顶。

而这个占 4 个字节的数值 3 就是立即数，下面是具体定义：

- **立即数**，也称**静态立即参数 / 静态操作数**，立即数是直接硬编码在指令里的（也就是字节码里），紧跟在操作码后面。大部分 Wasm 指令是没有立即数的，欲知 Wasm 指令中具体哪些指令是带有立即数的，请查阅项目中 `module.c` 文件的 `skip_immediate` 函数。
...
<img src="https://i.loli.net/2021/08/21/MuIbzj9Upnx8CKl.png" width=380/>
...
**调用栈**是由一个个独立的**栈帧**组成，每次函数调用，都会向调用栈压入一个栈帧（注意：为了阐述的简洁明了，仅讨论函数情况，其他例如 If / Loop 等控制块暂不在本文讨论中，读者可自行查阅项目中 `interpreter.c` 文件的 `interpreter` 函数）。每次函数执行结束，都会从调用栈弹出对应栈帧并销毁。一连串的函数调用，就是不停创建和销毁栈帧的过程。但在任一时刻，只有位于调用栈顶的栈帧是活跃的，也就是所谓的**当前栈帧**。

<img src="https://i.loli.net/2021/08/17/TyWARxZSklXBCPY.png" width=500/>
...
2. **操作数栈**，用于存储参数、局部变量、以及函数体指令执行过程中的操作数。
   ...
   这样做的好处是：调用方函数和被调用函数所关联的栈帧的操作数栈部分在整个操作数栈中是相邻的，便于调用方函数将参数传递给被调用函数，也便于被调用函数执行完成后将返回值传递给调用函数，具体的实现细节可以查阅项目中 `interpreter.c` 文件的 `interpreter` 函数。
3. **函数返回地址**，用于存储该栈帧调用指令的下一条指令的地址，当该栈帧从调用栈弹出时，会返回到该栈帧调用指令的下一条指令继续执行，换句话说就是当前栈帧对应的函数执行完退出后，返回到调用该函数的地方继续执行后面的指令。

<img src="https://i.loli.net/2021/08/17/VFXGQMbcDWBN3fR.png" width=500/>
...
下面这个 wat 文件中的有两个函数：compute 函数和 add 函数，其中 add 函数主要是接收两个数（类型分别是 32 位整数和 32 位浮点数），计算两数之和。compute 函数中调用了两次 add 函数，注意第二次调用 add 函数时，操作数栈上已经保存了上次调用 add 函数时的返回结果（再一次印证了两个函数关联的栈帧是共用同一个完整的操作数栈的，可以很便捷地实现函数之间参数的传递），所以这次仅需要传入第二个参数即可。
...
<img src="https://i.loli.net/2021/08/20/tqT7EaGKuRldJDW.png" width=500/>

最后展示下执行阶段对应的部分实际代码截图如下：

<img src="https://i.loli.net/2021/08/20/4fG29jlgXNL5unV.png" width=600/>

可以看到虚拟机的取指、译码、执行三个阶段，可以使用 while 循环和 switch-case 语句来简单地实现。更多细节建议查阅项目中 `interpreter.c` 文件的 `interpreter` 函数，其中有丰富的注释讲解。

## 结束语

以上就是 Wasm 解释器实现中的核心内容，当然这仅仅是 Wasm 解释器的最基本的功能--简单地逐条解析并执行指令，没有涉及更多高级功能例如 JIT、WASI 等。所以用该解释器解释执行 Wasm 文件，肯定没有 V8 加持下的 JS 实现相同功能时执行的速度快。

也正是由于其实现比较简单，所以源码更易读，并且其中有丰富的注释，所以非常适合有兴趣的读者快速了解 Wasm 核心原理。

需要指出的是，本篇文章并没有涉及到如何使用 Wasm 技术。恰好笔者正在基于 Wasm 和 FFMpeg 开发支持 H256 编码的视频播放器，相关文章链接如下：
